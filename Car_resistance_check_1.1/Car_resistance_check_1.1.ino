...
//***************************************************************************************************************************//
// Define IO Pin
#define SENSOR_RELAY_PIN_1 7
#define SENSOR_RELAY_PIN_2 6
#define SENSOR_RELAY_PIN_3 5
#define SENSOR_RELAY_PIN_4 4
...
const int RelayPinArray[4] = {SENSOR_RELAY_PIN_1, SENSOR_RELAY_PIN_2, SENSOR_RELAY_PIN_3, SENSOR_RELAY_PIN_4};
const int SensorInputPinArray[6] = {INPUT_PIN_RX1, INPUT_PIN_RX2, INPUT_PIN_RX3, INPUT_PIN_RX4, INPUT_PIN_RX5, CONSTANT_CURRENT__INPUT_PIN};

const double Rref = REF_VOL/(1023.0*MY_CONSTANT_CURRENT);
const double ConstantResistanceArray[6] = {R1, R2, R3, R4, R5, Rref};

const double MinXaxRes[2][NUM_OF_SENSOR] = {2.0, 6.0, 2.0, 2.0, 40.0, 12.0, 24.0, 24.0, 5.2,
...
void setup() {
  ...
  for (int i=0; i<4; i++) {
    pinMode(RelayPinArray[i], OUTPUT); digitalWrite(RelayPinArray[i], HIGH);
  }
  ...
  lcd.setCursor(0, 1); lcd.print("System V1.1 Made By:");
...
int CheckValue(int resNum, unsigned long StartTime) {
  ...
  int input_pin_num = SensorInputPinArray[resNum-1];
...
void  ManegeInput(int InputNum) {
  for (int i=0; i<4; i++) digitalWrite(RelayPinArray[i], HIGH);
  if (6<=InputNum && InputNum<=9) digitalWrite(RelayPinArray[InputNum-6], LOW);
