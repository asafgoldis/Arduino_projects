...
void setup(){
  ...
  for (int j=0; j<max_num_of_unit; ++j) {             // turn all relay's off
...
/*
  ++IntNum;
  if (IntNum==(2*60*10) || water[0][5]==LOW || water[1][5]==LOW || water[2][5]==LOW || water[3][5]==LOW || water[4][5]==LOW) {
    digitalWrite (0,HIGH);
    Serial.println("XXXXXXXXXXXXX");
    delay (5000);
    for (int j=0; j<5; j++) {                     //read sensor's
      soil_meas[j]=map(analogRead(SOIL_PIN[j]), 1019, 80, 0, 100);
      //Serial.print(", soil_meas U"); Serial.print(j+1); Serial.print(": "); Serial.print(soil_meas[j]);
    }
    IntNum=0;
  }
  else {
  ...
  //digitalWrite (0,HIGH);
  //delay (250);
  //Serial.print("SOIL_PIN is: "); Serial.print(analogRead(SOIL_PIN[0])); Serial.print(", IntNum is "); Serial.println(IntNum);
  //
  */
