...
*/
  ...
  for (int j=0; j<5; ++j) {             // turn all relay's off
...
void loop() {
  ...
  if (num_of_unit>=1) {soil_meas[0]=analogRead(SOIL_PIN[0]); soil_meas[0]=100-soil_meas[0]*100/1023;}
  if (num_of_unit>=2) {soil_meas[1]=analogRead(SOIL_PIN[1]); soil_meas[1]=100-soil_meas[1]*100/1023;}
  if (num_of_unit>=3) {soil_meas[2]=analogRead(SOIL_PIN[2]); soil_meas[2]=100-soil_meas[2]*100/1023;}
  if (num_of_unit>=4) {soil_meas[3]=analogRead(SOIL_PIN[3]); soil_meas[3]=100-soil_meas[3]*100/1023;}
  if (num_of_unit>=5) {soil_meas[4]=analogRead(SOIL_PIN[4]); soil_meas[4]=100-soil_meas[4]*100/1023;}
