...
//********************************//
#define MIN_TIME_TO_CHECK 15    // 15 min
#define POWER_PIN_HIGH_TIME 5      // 5 sec
#define DELTA 2               // +-2% 
//********************************//
#define TIME_TO_OFF_DISPLAY 15  // 10 sec
//********************************//
#define CALIBRATION_SOIL_SENSOR 991.0   // 991.1-1000.45  935
//********************************//
...
int unit_num = 1;
int num_of_unit = 0;
int minute_to_check = MIN_TIME_TO_CHECK;
...
void setup(){
  ...
  num_of_unit = 1;
  digitalWrite(POWER_PIN, POWER_PIN_ON); delay(1000);
...
void loop() {
  ...
  if ( water[0][5]==RELAY_ON || water[1][5]==RELAY_ON || water[2][5]==RELAY_ON || water[3][5]==RELAY_ON || water[4][5]==RELAY_ON || (currentSecond%(60*minute_to_check)<2) ) {      // 60*5 = 5 min
...
void Meas_Soil() {                                 //read soil sensor's
  for (int j=0; j<MAX_NUM_OF_UNIT; j++)
    soil_meas[j]=(1-analogRead(SOIL_PIN[j])/CALIBRATION_SOIL_SENSOR)*100.0;
}
...
void find_shortest_time() {
  int shortest_time = praneters[0][2];
  
  for(int i=1; i<num_of_unit; i++) {
    if (praneters[i][2] < shortest_time)
      shortest_time = praneters[i][2];
  }
   
  if (shortest_time >= MIN_TIME_TO_CHECK)
    minute_to_check = shortest_time;
  else
    minute_to_check = MIN_TIME_TO_CHECK;
}
