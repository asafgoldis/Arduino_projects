...
int acc_calibration_value = 1148;                            //Enter the accelerometer calibration value
...
void loop(){
  ...
  angle_gyro = angle_gyro * 0.9998 + angle_acc * 0.0002;                    //Correct the drift of the gyro angle with the accelerometer angle 0.9996+0.0004
  ...
  if(pid_setpoint == 0){                                                    //If the setpoint is zero degrees
    if(pid_output < 0)self_balance_pid_setpoint += 0.0025;                  //Increase the self_balance_pid_setpoint if the robot is still moving forewards 0.0015
    if(pid_output > 0)self_balance_pid_setpoint -= 0.0025;                  //Decrease the self_balance_pid_setpoint if the robot is still moving backwards 0.0015
  }
